...
const CrudOperation = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get('http://localhost:8999/api/customers/list');
  ...
  const addOrUpdateCustomer = async formData => {
    try {
      ...
      if (editCustomer) {
        await axios.post(`http://localhost:8999/api/customers/updateProfile?customerId=${editCustomer.id}&fullName=${fullName}&email=${email}`, data);
        setEditCustomer(null);
      } else {
        await axios.post('http://localhost:8999/api/customers/add', formData);
      }
  ...
  const deleteCustomer = async customerId => {
    try {
      await axios.delete(`http://localhost:8999/api/customers/delete/${customerId}`);
      fetchCustomers();
    } catch (error) {
